...
Generate a two-panel spacetime comparison:
1) Baseline simulation (full spatial resolution in ROI)
2) Experimental data (sensor resolution)
"""
...
def parse_args():
	parser = argparse.ArgumentParser(
		description="Generate baseline-vs-experimental spacetime plot.",
...
def save_plot(base_slip, base_positions, base_times, exp_slip, exp_times, sensor_positions, case_label, output_path):
	unified_vmin = float(min(base_slip.min(), exp_slip.min()))
	unified_vmax = float(max(base_slip.max(), exp_slip.max()))

	fig, axes = plt.subplots(1, 2, figsize=(14, 6), constrained_layout=True)
	fig.suptitle(f"Baseline vs Experimental: {case_label}", fontsize=14, fontweight="bold")
	...
	cbar1.set_label(r"Slip ($\mu$m)")
	cbar2 = fig.colorbar(im2, ax=axes[1], pad=0.02)
	cbar2.set_label(r"Slip ($\mu$m)")

	output_path.parent.mkdir(parents=True, exist_ok=True)
	fig.savefig(output_path, dpi=300, bbox_inches="tight")
	plt.close(fig)
	print(f"[SUCCESS] Saved spacetime plot to: {output_path}")
