#!/usr/bin/env python3
"""
Plot sparsity study results comparing different spatial data availability.
"""
...
def find_simulation_files(sim_name):
    ...
    # For sparsity study, the files are in sim_name_data_driven directories
    # but the files themselves are named with just sim_name
    
    # First try with _data_driven suffix for sparsity study results
    base_path_dd = os.path.join("simulation_outputs", f"{sim_name}_data_driven")
    if os.path.isdir(base_path_dd):
        base_path = base_path_dd
        actual_sim_name = sim_name  # Files are named without _data_driven
    else:
        # Try the exact name
        base_path = os.path.join("simulation_outputs", sim_name)
        actual_sim_name = sim_name
    ...
    coords_file = os.path.join(base_path, f"{actual_sim_name}.coords")
    data_dir = os.path.join(base_path, f"{actual_sim_name}-DataFiles")
...
if __name__ == "__main__":
    ...
    sim_dirs = glob.glob(os.path.join(sim_output_dir, "sparsity_*_percent"))
    if not sim_dirs:
        sys.exit("Error: No sparsity study simulation results found. Run run_disp_sparsity_study.py first.")

    sparsity_percentages = []
    for s_dir in sim_dirs:
        match = re.search(r"sparsity_(\d+)_percent", s_dir)
        if match:
            sparsity_percentages.append(int(match.group(1)))
    sparsity_percentages.sort()
    print(f"Found {len(sparsity_percentages)} sparsity study results: {sparsity_percentages[:10]}{'...' if len(sparsity_percentages) > 10 else ''}")

    # --- Create figure with new layout: displacement plots (top and bottom together) ---
    ...
    # --- Load Baseline (No Data-Driven / No Sparsity) ---
    ...
    # --- Color Map for Sparsity Percentages ---
    cmap = plt.cm.viridis
    norm = mpl.colors.Normalize(vmin=min(sparsity_percentages), vmax=max(sparsity_percentages))
    ...
    # --- Load and Plot Sparsity Study Data ---
    for sparsity_pct in sparsity_percentages:
        sim_name = f"sparsity_{sparsity_pct:03d}_percent"
        ...
        if X is None: 
            print(f"Warning: Could not load coordinates for {sim_name}")
            continue
        num_nodes = len(X)

        color = cmap(norm(sparsity_pct))
    ...
    print(f"Successfully loaded {len(results)} simulation results for plotting")
    ...
    if len(top_disps_020) > 1 and pos_020 is not None:
        ...
        if len(valid_top_020) > 1 and len(valid_bot_020) > 1:
            ...
            # Add markers: 'o' for start (lowest sparsity %), 'x' for end (highest sparsity %)
            # Start markers (lowest sparsity percentage - first in list)
            ...
            # End markers (highest sparsity percentage - last in list) 
    ...
    if len(top_disps_020) > 1 and pos_020 is not None:
        ...
        class EvolutionLineHandler(HandlerBase):
            def create_artists(self, legend, orig_handle, xdescent, ydescent, width, height, fontsize, trans):
                ...
                # Add 'x' marker at start (left side - should show 'x' for end/highest sparsity)
                ...
                # Add 'o' marker at end (right side - should show 'o' for start/lowest sparsity)
    ...
    cbar.set_label(r'Spatial Sparsity Fraction')
    ...
    fig_filename = os.path.join(figures_dir, 'sparsity_study_displacement.png')
    fig.savefig(fig_filename, dpi=300, bbox_inches='tight')
    print(f"Saved sparsity study displacement plots to: {fig_filename}")
    ...
    if X_gt is not None and gt_top_u0 is not None:
        ...
        for result in results:
            if result['top_u0'] is not None and result['bot_u0'] is not None:
                ...
                sparsity_list.append(result['sparsity_pct'])
                top_rmse_list.append(top_rmse)
                bot_rmse_list.append(bot_rmse)
        
        if sparsity_list:
            ax_rmse.semilogy(sparsity_list, top_rmse_list, 'bo-', label='Top Surface RMSE', markersize=6)
            ax_rmse.semilogy(sparsity_list, bot_rmse_list, 'ro-', label='Bottom Surface RMSE', markersize=6)
            
            # Find optimal sparsity (lowest RMSE)
            min_top_idx = np.argmin(top_rmse_list)
            optimal_sparsity = sparsity_list[min_top_idx]
            ax_rmse.axvline(optimal_sparsity, color='green', linestyle='--', alpha=0.7,
                          label=f'Optimal Sparsity: {optimal_sparsity}%')
            
            ax_rmse.set_xlabel('Sparsity Percentage (%)')
            ax_rmse.set_ylabel('RMSE vs Ground Truth [m]')
            ax_rmse.set_title('RMSE vs Spatial Data Sparsity')
            ax_rmse.grid(True, alpha=0.3)
            ax_rmse.legend()
            
            plt.tight_layout()
            
            rmse_filename = os.path.join(figures_dir, 'sparsity_study_rmse_analysis.png')
            fig_rmse.savefig(rmse_filename, dpi=300, bbox_inches='tight')
            print(f"Saved sparsity study RMSE analysis to: {rmse_filename}")
            
            print(f"\nOptimal sparsity level: {optimal_sparsity}% (min top RMSE: {min(top_rmse_list):.2e})")
        else:
